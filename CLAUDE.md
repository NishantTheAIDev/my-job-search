...
An AI-powered job search and application assistant. Fans out across multiple job boards (Adzuna, JSearch, LinkedIn, Indeed, Remotive, The Muse, Greenhouse, Lever, Ashby, Arbeitnow, Jobicy, We Work Remotely, Y Combinator), scores listings against the candidate's resume via Claude, tailors the resume and drafts cover letters, and lets the user review, edit, and **save** the result for reference. It does **not** submit to job boards. The app is **multi-user**: all candidate data is scoped per `user_id` and isolation is enforced at the service layer (hard invariant #1).
...
**Greenhouse, Lever, and Ashby** use free public unauthenticated APIs — no API keys needed. Configured via `GREENHOUSE_COMPANIES` / `LEVER_COMPANIES` / `ASHBY_COMPANIES` (comma-separated company slugs). Client-side query filtering uses **whole-word regex** (`\b` boundaries) so short terms like `"ai"` don't match substrings inside unrelated words (`"available"`, `"training"`). Ashby's endpoint is `https://api.ashbyhq.com/posting-api/job-board/{slug}` → `{"jobs": [...]}`; remote status comes from each job's `workplaceType` (Remote/Hybrid/Onsite) with `isRemote` as fallback, and `isListed:false` postings are skipped.

**Populating YC company slugs (Approach B)**: `scripts/refresh_yc_ats.py` is an **offline** maintenance tool (not called at search time) that pulls the hiring list from the free `yc-oss` API (`https://yc-oss.github.io/api/companies/hiring.json`), derives candidate slugs from each company's name/website, probes the Greenhouse/Lever/Ashby board APIs, and prints verified `GREENHOUSE_COMPANIES=` / `LEVER_COMPANIES=` / `ASHBY_COMPANIES=` lines to paste into `.env`. Run periodically (`uv run python scripts/refresh_yc_ats.py [--limit N] [--out file]`). Slug resolution is heuristic — a hit means the slug serves a public board, not necessarily *that* company's (name collisions possible) — so skim before committing. This is how broad YC coverage is achieved: the actual postings come through the existing per-slug ATS adapters, not a YC-specific search adapter. (Distinct from the `ycombinator` adapter below, which is the YC landing-page featured feed.)
...
**Y Combinator** (`https://www.ycombinator.com/jobs`) — public, keyless, no auth; robots.txt allows `/jobs`. The page is React-on-Rails: the job data is embedded as a JSON blob in a `data-page` attribute (`props.jobPostings`), which the adapter extracts and `json.loads` — it does **not** scrape HTML cards. **Limitation**: the landing page only exposes ~20 rotating *featured* jobs with **no server-side keyword/location filtering or pagination** (`?page=` is ignored; `/jobs/role/*` & `/jobs/location/*` are client-side React routes that 404 for non-browser clients). The full searchable index lives on workatastartup.com, which is login-gated. So this is a small supplementary feed: fetch once, filter client-side (whole-word query match over title+description, location substring, remote-only). Remote status is **inferred from the `location` text** (no remote flag). No full JD in the feed — `description` is synthesized from one-liner/role/type/skills and `fetch_description()` stays the base no-op; `url` points at the real posting. `posted_date` is a best-effort parse of a fuzzy relative age ("5 days", "about 1 month"). Gated by `settings.ycombinator_enabled` (default on).
...
5. **Board compliance**: Adzuna uses its free official API. JSearch uses the official RapidAPI endpoint. Greenhouse, Lever, and Ashby use their free public board APIs (no auth). LinkedIn uses the public guest search endpoint (no auth, HTML-parsed with `beautifulsoup4`). Remotive, The Muse, Arbeitnow, and Jobicy use their free public APIs. Indeed uses a public GraphQL endpoint with a hardcoded API key. We Work Remotely uses the public RSS feed (no auth). Y Combinator uses its public, robots-allowed `/jobs` landing page (no auth; JSON parsed from the embedded `data-page` blob).
